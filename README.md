...
Pytorch implementation of AnimeGAN for fast photo animation(相关场景数据集、pycharm完成复现源码以及训练权重参数在右边Releases目录中，可直接下载)
...
- **animeGAN.py** is Trained using unpaired data includes N photos and M animation images:
    + S(p) = {p(i) | i = 1, ..., N} ⊂ **P**
    + S(a) = {a(i) | i = 1, ..., M} ⊂ **A**
